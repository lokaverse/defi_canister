...
module {
  public shared ({ caller = owner }) actor class Miner({
    ...
    ///      - `hashrate`: The hashrate of the new miner (as `Nat`).
    ...
    /// 3. **Get Miner**:
    ///    - The function retrieves the miner associated with the wallet by calling `getMiner(wallet)` and stores it in the variable `miner`.
    /// 4. **Calculate Hashrate**:
    ///    - The function calculates the hashrate by multiplying `hashrate` by `1000000000000` and stores it in the variable `hash`.
    ...
    ///    - For each user, the function retrieves the miner's data using `miners.get(usr.1)` and stores it in the variable `miner`.
    ///    - It retrieves the miner's status using `minerStatus.get(miner_.id)` and stores it in the variable `minerStat`.
    ...
    ///    - It takes three parameters:
    ///      - `wallet`: The wallet address to which the ckBTC will be sent (as `Text`).
    ///      - `subAccount`: The sub-account associated with the wallet (as `Text`).
    ///      - `amount`: The amount of ckBTC to be transferred (as `Nat`).
    ///    - The function returns a `Bool` indicating the success (`true`) or failure (`false`) of the transfer.
    /// 2. **Convert Wallet Address**:
    ///    - The function converts the `wallet` text to a `Principal` type using `Principal.fromText(wallet)`.
    /// 3. **Admin Check**:
    ///    - The function ensures that the caller is an admin by calling `assert(_isAdmin(message.caller))`.
    /// 4. **Amount Check**:
    ///    - The function ensures that the amount to be transferred is greater than 0 by calling `assert(amount > 0)`.
    ...
    ///
    /// In summary, the `sendCKBTC` function transfers a specified amount of ckBTC to a given wallet, ensuring that the caller is an admin and the amount is greater than 0.
    /// It retrieves the current ckBTC balance, deducts a fixed fee, performs the transfer, and handles the transfer result.
    /// The function returns `true` if the transfer is successful and `true` by default (consider revising this logic).
    //
    ...
    ///      - `quoteId`: The quote ID associated with the withdrawal.
    ///      - `amount`: The amount of IDR to be withdrawn.
    ///      - `bankID`: The bank ID to which the IDR will be sent.
    ///      - `memoParam`: An array of `Nat8` representing the memo parameter.
    ...
    ///      - If the result is `#ok(m)`, it assigns the miner's ID to `id` and the miner's username to `usernm`.
    /// 8. **Encode Memo**:
    ///    - The function encodes the memo using `Text.encodeUtf8` and stores it in the variable `memo`.
    /// 9. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `minerStatus`.
    ...
    ///     - The function converts the memo parameter to a `Blob` using `Blob.fromArray` and stores it in the variable `blob`.
    /// 12. **Define CKBTC Actor**:
    ///     - The function defines an actor `CKBTC` with the method `icrc1_transfer`.
    ...
    ///
    /// In summary, the `withdrawIDR` function handles the withdrawal of IDR for a miner, ensuring that the system is not paused, the caller's address is verified, and the miner has sufficient balance.
    ...
    /// If any errors occur, it reverts the balance updates and returns an error.
    //
    ...
    ///      - If the result is `#ok(miner_)`, it means the miner exists, and the miner's data is stored in the variable `miner`.
    /// 5. **Initialize Revenue History and Total Revenue**:
    ///    - The function initializes `revenueHistory` as an empty array.
    ...
    ///    - If revenue history exists, the function updates `revenueHistory` and iterates over each entry in the revenue history:
    ...
    ///
    /// In summary, the `getTotalRevenue` function calculates the total revenue of a specific miner by retrieving the miner's revenue history and accumulating the revenue while excluding adjustments.
    /// It ensures that only admins can perform this operation and returns the total revenue as a `Nat` value.
    /// If the miner does not exist or has no revenue history, it returns 0.
    //
    ...
    ///    - For each user, the function retrieves the miner's data using `miners.get(usr.1)` and stores it in the variable `miner`.
    ///    - It initializes `revenueHistory` as an empty array.
    ...
    ///
    /// In summary, the `getAllRevenue` function calculates the total revenue of all miners by iterating through the list of users, retrieving each miner's revenue history, and accumulating the revenue while excluding adjustments.
    /// It ensures that only admins can perform this operation and returns the total revenue as a `Nat` value.
    //
    ...
    ///
    /// In summary, the `getAllBalance` function calculates the total balance of all miners by iterating through the list of users, retrieving each miner's balance, and accumulating the balances.
    /// It ensures that only admins can perform this operation and returns the total balance as a `Nat` value.
    //
    ...
    ///    - It takes three parameters:
    ///      - `username`: The username of the miner.
    ///      - `amount`: The amount of ckBTC to be withdrawn.
    ...
    ///      - If the result is `#ok(m)`, it assigns the miner's ID to `id` and the miner's username to `uname`.
    /// 7. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `minerStatus`.
    ...
    ///       - If the transfer fails (`#Err(msg)`), it reverts the balance updates, logs the failed transaction, and returns an error with the error message.
    ///
    /// In summary, the `withdrawCKBTC` function handles the withdrawal of ckBTC for a miner, ensuring that the system is not paused, the caller is verified, and the miner has sufficient balance.
    ...
    ///      - `action`: The action associated with the transaction.
    ///      - `amount`: The amount involved in the transaction.
    ///      - `txid`: The transaction ID.
    ///      - `currency`: The currency involved in the transaction.
    ///      - `username`: The username of the miner.
    ...
    ///      - `action`: The action associated with the transaction (`action`).
    ///      - `amount`: The amount involved in the transaction (`amount`).
    ///      - `txid`: The transaction ID (`txid`).
    ///      - `currency`: The currency involved in the transaction (`currency`).
    ...
    ///    - The function initializes an array `array` containing the created `transaction`.
    /// 6. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `status`.
    /// 7. **Update Miner Transactions**:
    ///    - The function appends the created `transaction` to the miner's existing transactions using `Array.append`, but only if `txid` is not "pre transfer".
    ...
    ///
    /// In summary, the `logTransaction` function logs a transaction for a specific miner by creating a transaction history record, updating the miner's transactions (if not a pre-transfer), and adding a detailed transaction log to the global list.
    /// It includes debug print statements to trace the logging process and ensures that the transaction index is incremented to maintain unique transaction IDs.
    //
    ...
    ///    - The function increments the `transactionIndex` by 1 to ensure that the next transaction will have a unique ID.
    ///
    /// In summary, the `withdrawalLog` function logs a withdrawal transaction for a specific miner by creating a transaction history record, updating the miner's transactions, and adding a detailed transaction log to the global list.
    /// It includes debug print statements to trace the logging process and ensures that the transaction index is incremented to maintain unique transaction IDs.
    //
    ...
    ///      - `id`: The ID of the distribution (not used in the function body).
    ///      - `action`: The action associated with the distribution.
    ///      - `amount`: The amount involved in the distribution.
    ///      - `txid`: The transaction ID.
    ///      - `currency`: The currency involved in the distribution.
    /// 2. **Debug Print**:
    ...
    ///      - `id`: The current value of `transactionIndex`.
    ///      - `time`: The current time obtained by calling `now()`.
    ///      - `action`: The action associated with the distribution (`action`).
    ///      - `amount`: The amount involved in the distribution (`amount`).
    ///      - `txid`: The transaction ID (`txid`).
    ///      - `currency`: The currency involved in the distribution (`currency`).
    ///      - `provider` and `receiver`: Not used in this context (commented out).
    /// 4. **Debug Print**:
    ...
    ///    - The function increments the `transactionIndex` by 1 to ensure that the next transaction will have a unique ID.
    ///
    /// In summary, the `logDistribution` function logs a distribution transaction by creating a transaction history record with the distribution details and adding it to the global list of transactions.
    /// It also increments the transaction index to maintain unique transaction IDs and includes debug print statements to trace the logging process.
    //
    ...
    ///    - The function initializes an array `array` containing the created `transaction`.
    /// 5. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus_.get(id_)` and stores it in the variable `status`.
    ...
    ///
    /// In summary, the `forcelogTransaction` function logs a transaction for a specific miner by creating a transaction history record, updating the miner's transactions, and adding the transaction to the global list.
    /// It ensures that only admins can perform this operation and returns the updated list of transactions for the miner.
    //
    ...
    ///      - `id`: The ID of the miner.
    ///      - `username`: The username of the miner.
    ///      - `hash`: The hashrate of the miner.
    ///      - `wallet`: The wallet address of the miner.
    ...
    ///      - `amount`: The hashrate of the miner (`hash`).
    ///      - `txid`: The username of the miner (`username`).
    ...
    ///    - The function increments the `transactionIndex` by 1 to ensure that the next transaction will have a unique ID.
    ///
    /// In summary, the `logMiner` function logs the creation of a new miner by creating a transaction history record with the miner's details and adding it to the list of transactions.
    /// It also increments the transaction index to maintain unique transaction IDs.
    //
    ...
    ///    - It takes four parameters:
    ///      - `username`: The username of the miner.
    ///      - `amount`: The amount of USDT to be withdrawn.
    ///      - `addr`: The address to which the USDT will be sent.
    ///      - `usd`: The amount of USDT in USD.
    ///    - The function returns a `T.TransferRes` indicating the result of the withdrawal.
    /// 2. **Pause Check**:
    ///    - The function ensures that the system is not paused by calling `assert(_isNotPaused())`.
    /// 3. **Maintenance Check**:
    ...
    ///      - If the result is `#ok(m)`, it assigns the miner's ID to `id`.
    /// 7. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `minerStatus`.
    /// 8. **Check Sufficient Balance**:
    ///    - The function checks if the miner has sufficient balance to cover the withdrawal amount plus a fee of 10 units.
    ...
    ///     - It constructs a unique ID `uid` and a URL for the HTTP request.
    /// 11. **Update Balances**:
    ...
    ///    - It takes a single parameter `amount` of type `Nat`, which represents the amount of ckBTC to be transferred.
    ///    - The function returns a `Bool` indicating the success (`true`) or failure (`false`) of the transfer.
    /// 2. **Pause Check**:
    ///    - The function ensures that the system is not paused by calling `assert(_isNotPaused())`.
    /// 3. **Initialize Amount**:
    ///    - The function assigns the input parameter `amount` to a local variable `amountNat`.
    /// 4. **Perform Transfer**:
    ...
    ///
    /// In summary, the `moveCKBTC` function transfers a specified amount of ckBTC from one vault to another, handling various error cases and ensuring that the system is not paused before performing the transfer.
    /// It returns `true` if the transfer is successful and `false` otherwise.
    //
    ...
    /// 1. **Function Signature**:
    ///    - The function is declared as `async`, meaning it performs asynchronous operations.
    ///    - It takes a single parameter `url` of type `Text`, which represents the URL to which the HTTP request will be sent.
    ...
    /// 3. **Set URL**:
    ///    - The function assigns the input parameter `url` to a local variable `url`.
    ...
    ///
    /// In summary, the `send_http` function sends an HTTP GET request to a specified URL with predefined headers, processes the response, and returns the response body as a `Text`.
    /// It includes error handling for decoding the response body and adds cycles to ensure the request can be processed.
    //
    ...
    ///    - It takes a single parameter `nat` of type `Nat`.
    /// 2. **Convert Nat to Nat64**:
    ///    - The function converts the `Nat` value `nat` to a `Nat64` value using `Nat64.fromNat(nat_)`.
    ///    - The result is stored in the variable `toNat64`.
    /// 3. **Convert Nat64 to Int64**:
    ///    - The function converts the `Nat64` value `toNat64` to an `Int64` value using `Int64.fromNat64(toNat64_)`.
    ///    - The result is stored in the variable `toInt64`.
    /// 4. **Convert Int64 to Float**:
    ///    - The function converts the `Int64` value `toInt64` to a `Float` value using `Float.fromInt64(toInt64_)`.
    ///    - The result is stored in the variable `amountFloat`.
    /// 5. **Return Float**:
    ///    - The function returns the `Float` value `amountFloat`.
    ///
    /// In summary, the `natToFloat` function converts a `Nat` value to a `Float` value by first converting it to `Nat64`, then to `Int64`, and finally to `Float`.
    /// This conversion process ensures that the `Nat` value is accurately represented as a floating-point number.
    //
    ...
    /// 1. **Function Signature**:
    ///    - The function takes a single parameter `wallet` of type `Principal`, which represents the wallet principal of the miner.
    ...
    ///
    /// In summary, the `getMiner` function retrieves the miner associated with a given wallet principal from the `minerHash` hash map.
    /// It returns `#ok` with the miner data if the miner is found, or `#none` if the miner is not found.
    //
    ...
    ///
    /// In summary, the `getUsername` function retrieves all usernames and their corresponding IDs, ensuring that only admins can perform this operation.
    /// It returns the data in an array of tuples format.
    //
    ...
    ///    - It takes two parameters: `userName` of type `Text` and `hashPercent` of type `Nat`.
    ...
    /// 2. **Input Validation**:
    ///    - The function checks if `hashPercent` is within the valid range (0, or between 100 and 10000). If not, it returns `#failed("Must 0, less than 100 and more than 1%")`.
    ...
    ///    - If the miner exists, it assigns the miner's username to `callerName`.
    /// 5. **Self-Share Check**:
    ...
    ///    - It creates a `share` object containing the target user's name, wallet address, and share percentage.
    ...
    ///      - If the result is `#ok(m)`, it means the miner exists, and the miner's ID is stored in the variable `id`.
    /// 5. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `status`.
    /// 6. **Retrieve Revenue History**:
    ///    - The function initializes `revenueHistory` as an empty array and `yesterdayRevenue` as 0.
    ///    - It checks if there is revenue history for the miner using `revenueHash.get(Principal.toText(m.walletAddress))`.
    ///    - If revenue history exists, it updates `revenueHistory` and sets `yesterdayRevenue` to the last entry's sats value.
    /// 7. **Retrieve Current Shared Revenue**:
    ///    - The function initializes `currentShared` as 0 and `shareList` as an empty list.
    ///    - It checks if there are revenue sharing rules for the miner using `revenueShareHash.get(m.username)`.
    ///    - If revenue sharing rules exist, it updates `currentShared` and `shareList` with the relevant data.
    /// 8. **Retrieve Received Shared Revenue**:
    ///    - The function initializes `receivedShareList` as an empty list.
    ///    - It checks if there are received revenue sharing rules for the miner using `receivedRevenueShareHash.get(m.username)`.
    ///    - If received revenue sharing rules exist, it updates `receivedShareList` with the relevant data.
    /// 9. **Adjust Balance**:
    ...
    ///
    /// In summary, the `getMinerData` function retrieves detailed information about the miner associated with the caller, including their status, revenue history, shared revenue, and wallet addresses.
    /// It ensures that the caller's address is verified and returns the miner's data in a structured format.
    /// If the miner is not found or not verified, it returns `#none`.
    //
    ...
    ///     - The function returns `#ok(minerData)` to indicate that the miner was found and includes the detailed miner data.
    ///
    /// In summary, the `fetchMinerByPrincipal` function retrieves detailed information about a miner, including their status, revenue history, shared revenue, and wallet addresses, based on the provided principal.
    /// It ensures that the caller is an admin and returns the miner's data in a structured format.
    /// If the miner is not found, it returns `#none`.
    //
    ...
    ///    - The function retrieves the miner's data using `miners.get(p)` and stores it in the variable `miner`.
    ///    - It retrieves the miner's status using `minerStatus.get(miner_.id)` and stores it in the variable `status`.
    ...
    ///
    /// In summary, the `adjustB` function updates the balance of a miner based on their username, ensuring that the caller is an admin and that the miner exists.
    /// It retrieves the miner's data and status, updates the balance, and returns the new balance.
    //
    ...
    /// 5. **Update Miner Balance**:
    ///    - The function retrieves the miner's status using `minerStatus.get(miner_.id)` and stores it in the variable `status`.
    ...
    ///
    /// In summary, the `adjustMiner` function updates the username and balance of a miner, ensuring that the caller is an admin and that the miner exists.
    /// If the miner is found and updated successfully, it returns 1; otherwise, it returns 0.
    //
    ...
    ///    - The function iterates over each query in `datas`.
    ...
    ///    - It retrieves the miner's ID using `usernameHash.get(uname)` and stores it in `p`.
    /// 6. **Get Miner and Status**:
    ///    - The function retrieves the miner's data using `miners.get(p)` and stores it in `miner`.
    ...
    ///
    /// In summary, the `adjustMinerBalance` function processes a set of queries to adjust the balances of miners, handles revenue sharing, updates the relevant records, and returns the result of the adjustments.
    /// It ensures that only admins can perform the adjustments and logs the changes appropriately.
    //
    ...
    ///    - The function initializes `yesterdayRevenue` to 0 and `p` to 0.
    /// 4. **Get Miner ID**:
    ...
    ///     - The function returns `#ok(minerData)` to indicate that the miner was found and includes the detailed miner data.
    ///
    /// In summary, the `fetchMinerByUsername` function retrieves detailed information about a miner, including their status, revenue history, shared revenue, and wallet addresses, based on the provided username.
    /// It ensures that the caller is an admin and returns the miner's data in a structured format.
    /// If the miner is not found, it returns `#none`.
    //
    ...
    ///
    /// In summary, the `fetchMinerById` function retrieves detailed information about a miner, including their status, revenue history, shared revenue, and wallet addresses, based on the provided miner ID.
    /// It ensures that the caller is an admin and returns the miner's data in a structured format.
    //
    ...
    /// 1. **Function Signature**:
    ///    - The function is declared as `public query`, meaning it can be called by external actors and does not modify the state.
    ///    - It takes a single parameter `id` of type `Nat` (natural number), which represents the ID of the miner.
    ///    - The function returns an array of `T.WalletAddress` asynchronously.
    /// 2. **Get Miner Status**:
    ///    - The function retrieves the miner's status using `minerStatus.get(id_)` and stores it in the variable `miner`.
    ...
    ///
    /// In summary, the `getWallets` function retrieves and returns the wallet addresses associated with a specific miner based on the provided miner ID.
    /// It ensures that the function is a query, meaning it does not modify the state and can be called by external actors.
    //
    ...
    ///    - The function is declared as `public shared`, meaning it can be called by external actors.
    ///    - It takes three parameters:
    ///      - `name`: The name associated with the wallet address.
    ///      - `address`: The wallet address.
    ///      - `currency`: The currency associated with the wallet address.
    ...
    ///    - The function creates a new wallet address record with the provided details and stores it in the variable `wallet`.
    ...
    ///
    /// In summary, the `saveWalletAddress` function saves a new wallet address for the miner associated with the caller, ensuring that the caller's address is verified and that the wallet address does not already exist.
    /// If the miner does not exist, it returns `false`; otherwise, it saves the wallet address and returns `true`.
    //
    ...
    ///      - `jwalletId`: The jwallet ID associated with the bank account.
    ///    - The function returns a `Bool` indicating success (`true`) or failure (`false`).
    /// 2. **Verify Address**:
    ///    - The function ensures that the caller's address is verified by calling `assert(_isAddressVerified(message.caller))`.
    /// 3. **Get Miner**:
    ...
    ///    - The function creates a new bank address record with the provided details and stores it in the variable `bank`.
    ...
    ///
    /// In summary, the `saveBankAddress` function saves a new bank address for the miner associated with the caller, ensuring that the caller's address is verified and that the bank account does not already exist.
    /// If the miner does not exist, it returns `false`; otherwise, it saves the bank address and returns `true`.
    //
    ...
    ///
    /// In summary, the `textToFloat` function processes each character in the input text, converting digits to their corresponding floating-point values and handling decimal points appropriately.
    /// If an invalid character is encountered, it returns `0.0`.
    //
    ...
    ///
    /// In summary, the `setBalance` function updates the balance of the miner associated with the caller, ensuring that the miner exists before performing the update.
    /// If the miner does not exist, it returns `false`; otherwise, it sets the balance and returns `true`.
    ...
    /// 1. **Get Current Time**:
    ///    - The function retrieves the current time using `now()` and stores it in the variable `now`.
    ...
    ///    - The function initializes `hashrateRewards` as an empty string and `count` as 0.
    ...
    ///      - `word`: The text to be split.
    ///      - `delimiter`: The character used as the delimiter for splitting the text.
    /// 2. **Splitting the Text**:
    ///    - The function uses `Text.split` to split the input text (`word`) by the specified delimiter (`delimiter`).
    ...
    //// 1. **Input Parsing**:
    ////    - The function takes a `rewards` string as input, which contains reward distribution data.
    ////    - It splits the `rewards` string into components using the `textSplit` function.
